...
// Streaming chat — sends response chunks as SSE events
router.post("/:userId/chat/stream", async (req, res) => {
  ...
  const cleanup = () => {
    ...
    if (abortExec) abortExec();
  };

  // Handle client disconnect
  req.on("close", cleanup);

  abortExec = streamExecInContainer(
    info.containerId,
    ["openclaw", "agent", "--agent", "main", "--message", message, "--local"],
    {
      onData(text) {
      ...
      onEnd(fullOutput) {
        ...
        console.log(
          ...
          userId.slice(0, 8),
          ":",
          fullOutput.slice(0, 100),
        );
        res.write(`data: ${JSON.stringify({ type: "done", content: fullOutput })}\n\n`);
        res.write("data: [DONE]\n\n");
        cleanup();
        res.end();
      },
      onTimeout() {
        res.write(`data: ${JSON.stringify({ type: "error", content: "Command timeout" })}\n\n`);
        res.write("data: [DONE]\n\n");
        cleanup();
        res.end();
      },
