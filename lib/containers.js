...
/**
 * Stream exec output from a container command as a callback per chunk.
 * Unlike execInContainer which buffers everything, this yields each stdout
 * chunk as it arrives from the Docker stream.
 *
 * @param {string} containerId - Docker container ID
 * @param {string[]} cmd - Command to execute
 * @param {object} callbacks - { onData(text), onStatus(text), onError(text), onEnd(fullOutput), onTimeout() }
 * @param {number} timeoutMs - Timeout in milliseconds
 * @returns {function} abort function to cancel the exec
 */
export function streamExecInContainer(containerId, cmd, callbacks, timeoutMs = 120000) {
  const { onData, onStatus, onError, onEnd, onTimeout } = callbacks;
  let output = "";
  let errorOutput = "";
  let finished = false;
  ...
  const finish = () => {
    ...
    finished = true;
    clearTimeout(timer);
    onEnd?.(output.trim(), errorOutput.trim());
  };
    ...
    .then((stream) => {
      stream.on("data", (chunk) => {
        ...
        if (chunk.length > 8) {
          ...
          if (streamType === 2) {
            errorOutput += data;
            // Detect status-like stderr (tool invocations, skill calls)
...
/**
 * Detect if a stderr line looks like a status/progress message rather than
 * an actual error. Openclaw emits tool invocations and skill progress to stderr.
 */
function isStatusMessage(text) {
  const lower = text.toLowerCase().trim();
  // Common patterns: "Running tool: ...", "Calling skill: ...", "Processing...", "[tool]"
  return (
    lower.includes('running tool') ||
    lower.includes('calling skill') ||
    ...
    lower.startsWith('>')  // openclaw often prefixes status with >
  );
}
